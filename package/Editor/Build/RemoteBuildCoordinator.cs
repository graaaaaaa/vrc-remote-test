    ...
    public sealed class RemoteBuildCoordinator
    {
            ...
            IProgress<RemoteBuildProgress> progress, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                // synchronously up to its first await). PollForResultAsync
                // runs after several ConfigureAwait(false) hops and may no
                // longer be on the main thread, so it cannot safely call
                // EditorPrefs itself (EditorPrefs.GetInt throws if called off
                // the main thread) — pass the values in instead.
